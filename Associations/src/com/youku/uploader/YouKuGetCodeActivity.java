...
public class YouKuGetCodeActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.activity_youku_request_code);
		// 设置Web视图
		webview = (WebView) findViewById(R.id.wb_youku);
		if (webview == null) {
			return;
		}
		...
		webview.setWebViewClient(new WebViewClient() {
			...
			public boolean shouldOverrideUrlLoading(WebView view, String url) {
				...
				Log.d(TAG, "code=" + strCode + " status=" + str[1] + " " + url
						+ " token=" + getToken());

				Toast.makeText(getApplicationContext(),
						strCode + "access_token=" + access_token, 0).show();
				it.setClass(YouKuGetCodeActivity.this, YoukuMainActivity.class);
				startActivity(it);
				return false;
			}
		});
		...
		Thread t = new Thread(new Runnable() {
			...
			public void run() {
				...
				params.add(new BasicNameValuePair("client_secret",
						CLIENT_SECRET));// 你申请优酷的client_secret
				...
				try {
					...
					if (httpResponse.getStatusLine().getStatusCode() == 200) {
						...
						// Toast.makeText(getApplicationContext(),
						// "access_token="+access_token, 0).show();
						// return access_token;
					} else {
						Log.d(TAG, "getToken() getStatusCode="
