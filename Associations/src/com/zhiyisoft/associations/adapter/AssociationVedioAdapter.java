...
public class AssociationVedioAdapter extends BAdapter {
	...
	private void bundledataToView(int position, ViewHolder holder) {
		...
		if (holder != null && works != null) {
			...
			holder.tv_vedio_date.setText(DateUtil.stamp2humanDate(works
					.getCtime()));
		}
	 ...
	 * @return
	 */
	private List<Model> getVideo(int index) {
		if (mLeague != null) {
			mLeague.setP(index);
